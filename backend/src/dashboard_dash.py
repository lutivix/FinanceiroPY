...
Filtros funcionando em tempo real + Categorização Inline!
"""
...
def carregar_dados():
    """Carrega dados do banco de dados (exceto INVESTIMENTOS, SALÁRIO e transferências internas)"""
...
def carregar_transacoes_pendentes():
    """Carrega apenas transações 'A definir' para categorização (excluindo transferências internas)"""
    conn = sqlite3.connect(DB_PATH)
    query = """
    SELECT 
        rowid as row_id,
...
def atualizar_categoria_banco(rowid, nova_categoria):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE transacoes_openfinance SET categoria = ? WHERE rowid = ?",
...
def obter_categorias_disponiveis():
    ...
    conn = sqlite3.connect(DB_PATH)
    query = "SELECT DISTINCT categoria FROM transacoes_openfinance WHERE categoria NOT IN ('INVESTIMENTOS', 'SALÁRIO', 'A definir') ORDER BY categoria"
    df = pd.read_sql_query(query, conn)
    conn.close()
...
df_pendentes = carregar_transacoes_pendentes()
...
def atualizar_secao_pendentes(refresh):
    ...
    # Calcular totais (excluindo transferências internas)
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("""
        SELECT COUNT(*) FROM transacoes_openfinance 
        WHERE tipo_transacao = 'DEBIT' 
          AND categoria NOT IN ('INVESTIMENTOS', 'SALÁRIO')
    ...
    for idx, row in df_pend.iterrows():
        ...
        rows.append(
            html.Tr([
                html.Td(row['data'].strftime('%d/%m/%Y'), style={'width': '10%'}),
                html.Td(row['descricao'], style={'width': '35%', 'maxWidth': '300px', 'overflow': 'hidden', 'textOverflow': 'ellipsis'}),
                html.Td(f"R$ {row['valor_normalizado']:,.2f}", style={'width': '12%', 'textAlign': 'right'}),
                html.Td(row['fonte'], style={'width': '15%'}),
                html.Td([
                    dcc.Dropdown(
...
def atualizar_dashboard(mes_selecionado, categoria_selecionada, fonte_selecionada, refresh):
    ...
    if mes_selecionado != 'TODOS':
        ...
        if diferenca_mes > 0:
            ...
            card_diferenca_border_style = {'border': '2px solid #dc3545'}
        else:
            # Economizou
            card_diferenca_mes = html.Div([
            ...
            card_diferenca_border_style = {'border': '2px solid #28a745'}
    ...
    if mostrar_graficos_temporais:
        ...
        fig_evolucao.add_trace(go.Bar(
            ...
            text=[f'{v/1000:.1f}k' if v >= 1000 else f'R$ {v:.0f}' for v in gastos_12meses.values],
